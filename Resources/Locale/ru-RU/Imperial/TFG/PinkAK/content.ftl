ent-WeaponRifleAkPink = УВУМС
    .desc = Исповьзует патвоны .30 винтовочные QwQ
    .suffix = Адмемы
ent-WeaponRifleAkPinkDebug = УВУМС
    .desc = Исповьзует особые патвоны, котовые девают QwQ
    .suffix = Дебаг, Адмемы

reagent-name-uwu = Сок, который делает вас UwU!
reagent-desc-uwu = Заставляет цель увукать. При достижении 50 унций временно превращает в котика.
flavor-uwu = как уву
chat-emote-name-uwu = Увукать
chat-emote-msg-uwu = увукает

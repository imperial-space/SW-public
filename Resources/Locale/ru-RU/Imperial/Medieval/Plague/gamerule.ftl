medieval-plague-title = Испытание Бога: Калинда
medieval-plague-description = На земли опустилась чума! Переживите и излечите эту напасть, пока не стало слишком поздно!

roles-antag-plague-name = Призрак чумы
roles-antag-plague-objective = Вместе с другими призраками заразите чумой как можно больше людей.

plague-intro-text = Вы - один из призраков чумы, насланной на эти земли. Кооперируйтесь и заразите смертельной болезнью всех, кого сможете.

medieval-plague-symptom-unlocked = Был разблокирован симптом { $symptom }.

medieval-plague-round-end-infected-count = Всего заражённых: [color=crimson]{ $count }[/color]
medieval-plague-round-end-immune-count = Иммунных к чуме: [color=crimson]{ $count }[/color]
medieval-plague-round-end-plague-tier = Тяжесть болезни: [color=crimson]{ $tier }[/color]
medieval-plague-round-end-symptoms-count = Разблокировано симптомов: [color=crimson]{ $count }[/color]

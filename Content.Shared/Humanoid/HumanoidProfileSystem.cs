...
public sealed class HumanoidProfileSystem : EntitySystem
{
    ...
    /// <summary>
    /// Imperial Medieval: sets species without full profile apply (e.g. nocturn disguise).
    /// </summary>
    public void SetSpecies(Entity<HumanoidProfileComponent?> ent, ProtoId<SpeciesPrototype> species)
    {
        if (!Resolve(ent, ref ent.Comp))
            return;

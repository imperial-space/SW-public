...
public abstract partial class SharedGunSystem
{
    ...
    private void OnBallisticInteractUsing(Entity<BallisticAmmoProviderComponent> ent, ref InteractUsingEvent args)
    {
        ...
        if (GetBallisticShots(ent.Comp) >= ent.Comp.Capacity)
            return;
        // Imperial Medieval Event start
        if (ent.Comp.CartridgeFillDelay > TimeSpan.Zero)
        {
            Audio.PlayPredicted(ent.Comp.SoundInsert, ent.Owner, args.User);
            _doAfter.TryStartDoAfter(new DoAfterArgs(EntityManager, args.User, ent.Comp.CartridgeFillDelay, new CartridgeReloadDoAfterEvent(), used: args.Used, target: ent.Owner, eventTarget: ent.Owner)
            {
                BreakOnMove = true,
                BreakOnDamage = true,
                NeedHand = true,
    ...
    private void OnCartridgeReloadDoAfter(EntityUid uid, BallisticAmmoProviderComponent component, CartridgeReloadDoAfterEvent args)
    {
        ...
        UpdateBallisticAppearance((uid, component)); // Imperial Medieval

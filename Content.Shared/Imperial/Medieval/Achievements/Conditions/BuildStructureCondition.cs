...
public sealed partial class BuildStructureCondition : AchievementCondition
{
    ...
    public override FormattedMessage GetDescription(IPrototypeManager protoManager)
    {
        ...
        msg.AddText(Loc.GetString("achievement-condition-build-structure",
    ...
    {
        if (!CheckFilters(player, entManager, protoManager) || context is not EntityUid builtEnt)
            return false;

        var meta = entManager.GetComponent<MetaDataComponent>(builtEnt);

...
public sealed partial class RefillBarrierCondition : AchievementCondition
{
    public override FormattedMessage GetDescription(IPrototypeManager protoManager)
    {
        ...
        msg.AddText(Loc.GetString("achievement-condition-refill-barrier", 
            ("count", RequiredCount)));
    ...
    public override bool TryUpdateProgress(EntityUid player, IEntityManager entManager, IPrototypeManager protoManager, object? context, Dictionary<string, int> progress)
    {
        ...
        if (context is not BarrierRefilledContext)
            return false;
...
public record BarrierRefilledContext;
...
public sealed partial class CompleteMercContractCondition : AchievementCondition
{
    public override FormattedMessage GetDescription(IPrototypeManager protoManager)
    {
        var msg = new FormattedMessage();
        msg.PushColor(Color.FromHex("#9e8c78"));
        msg.AddText(Loc.GetString("achievement-condition-complete-contract",
            ("count", RequiredCount)));
        msg.Pop();
        ...
        Dictionary<string, int> progress)
    {
        ...
        if (context is not MercenaryContractCompletedContext)
            return false;
...
public record MercenaryContractCompletedContext;

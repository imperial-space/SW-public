...
namespace Content.Server.Database
{
    public abstract class ServerDbBase
    {
        ...
        private readonly ISerializationManager _serialization; // Imperial Medieval / public organ markings
        ...
        {
            // Imperial Medieval: port ConvertProfiles onto organ-markings HumanoidCharacterAppearance
            ...
            else if (profile.Markings is { } profileMarkings)
            {
                // Legacy flat markings list — keep empty organ map if conversion manager unavailable here
                List<Marking> markingsList = new();
                var markingsRaw = profileMarkings.Deserialize<List<string>>();
                if (markingsRaw != null)
                {
                ...
                // Imperial Medieval: ConvertMarkings needs MarkingManager; leave empty if only legacy data
                _ = markingsList;
            }
